...
dns_file=$pcap_dir/dns_pairs.txt
windowed_file=$pcap_dir/windowed/$filename.csv
...
if [ -f "$windowed_file" ]; then
	...
	python3 $mliotrim_dir/pcap_to_features.py $device $window_duration $csv_file $windowed_file $dns_file
fi
